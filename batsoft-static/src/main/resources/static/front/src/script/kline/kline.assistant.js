...
Kline.prototype = {
        ...
        $.getJSON(c, function (g) {
            ...
            if (l != null && l.length > 0) {
                ...
                for (var k = l.length - 1; k >= 0; k--) {
                    ...
                    var h = d.fixNumber(l[k].amount, d.coinDecimal[d.symbol]);
                    var f = h.split(".");
    ...
    updateDepth: function (c) {
        ...
        if (this.lastDepth == null) {
            ...
            var k = $(window.top.document.body).find("#sellUnitPrice"), f = $(window.top.document.body).find("#buyUnitPrice");
            if (f.val() == "") {
                try {
    ...
    getAmount: function (a) {
        ...
        var b = a[0] + "." + a[1];
        b = this.fixNumber(parseFloat(b), this.coinDecimal[this.symbol]);
        var b = b.split(".");
        return [b[0], b[1]]
    },
    ...
    refreshUrl: function (a) {
        try {
            ...
            History.pushState({state: this.browerState}, this.title, "?symbol=" + a)
        } catch (b) {
        }
    },
    isBtc123: function () {

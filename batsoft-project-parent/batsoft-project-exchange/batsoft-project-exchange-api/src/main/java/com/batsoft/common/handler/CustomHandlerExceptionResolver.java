...
public class CustomHandlerExceptionResolver implements HandlerExceptionResolver {
	...
	protected void buildReturnValue(HttpServletRequest request, HttpServletResponse response, Object handler, Map<String, String> notification) throws IOException {
		...
		try {
			...
			ApiResultData<Object> result = new ApiResultData<Object>();
			result.setStatus(ResponseStatusCodeEnum.FAIL.getValue());
			ResponseBody responseBody = ((HandlerMethod)handler).getMethodAnnotation(ResponseBody.class);
			if(responseBody != null){
				out.print(instance.toJson(result));
			}else {
				// request.getSession().setAttribute(KeyEnum.message.name(), instance.toJson(result));
				// request.getRequestDispatcher(CHS.left_slash.getValue()).forward(request, response);
			}
		}catch (Exception e) {
			// 
		} finally {

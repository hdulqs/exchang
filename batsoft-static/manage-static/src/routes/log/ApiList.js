...
@Form.create()
export default class ApiList extends BaseList {
  state =  this.state;
  columns = [
    {
      dataIndex: 'url',
    ...
    },
    {
      ...
      render: val => (
        <Fragment>
          <Link className={this.hasPermissions("log:api:see")?"":styles.hidden} to={{pathname: `/log/api/info/${val}`}}>查看</Link>
          <Divider type="vertical"/>
          <Link className={this.hasPermissions("log:api:edit")?"":styles.hidden} to={{pathname: `/log/api/edit/${val}`}}>编辑</Link>
        </Fragment>
      ),
    },
  ];
  ...
  handleMenuClick = (e) => {
    ...
    switch (e.key) {
      case 'remove':
        this.remove("api/remove",params,this.removeCallBack);
        break;
      default:
        break;
    }
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
          <Row gutter={{md: 8, lg: 24, xl: 48}}>
              ...
              <Button style={{marginLeft: 8}} onClick={() => this.handleFormReset("api/fetch")}>重置</Button>
   ...
   */
  render() {
    const {api: {list}, loading} = this.props;
    const {selectedRows, modalVisible} = this.state;

    const menu = (
      <Menu onClick={this.handleMenuClick} selectedKeys={[]}>
          <Menu.Item key="remove" className={this.hasPermissions("log:api:remove")?"":styles.hidden}>批量删除</Menu.Item>
      </Menu>
    );

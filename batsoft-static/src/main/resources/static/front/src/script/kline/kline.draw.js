...
var GLOBAL_VAR = {
    ...
    mark_from         :  "11",
    limit             :  "1000",
    requestParam      :  "marketFrom=11&type=15min&limit=1000",
    periodMap	      :  null,
    chartMgr          :  null,
    G_HTTP_REQUEST    :  null,
    TimeOutId 		  :  null,
    ...
    Response : function(marketFrom, type, coinVol, data) {
        ...
        if (ChartManager.getInstance().getChart()._money_type == 1) {
            ...
            for (var i in GLOBAL_VAR.KLineData) {
                ...
                e[1] = parseFloat((e[1] * rate).toFixed(GLOBAL_VAR.digits));
                e[2] = parseFloat((e[2] * rate).toFixed(GLOBAL_VAR.digits));
                e[3] = parseFloat((e[3] * rate).toFixed(GLOBAL_VAR.digits));
                // 侧栏收盘价
                e[4] = parseFloat((e[4] * rate).toFixed(GLOBAL_VAR.digits));
            }
        }
        try {
        ...
        if (Pushing.state == Pushing.State.Enable) {
            ...
            if (f != -1) {
                if (!Pushing.PushFrom)
                    return;
                // 是否为分时
                var isLine = $("#chart_toolbar_periods_vert ul a.selected").parent().attr('name') == 'line';
                Pushing.PushFrom(marketString, Pushing.marketFrom, Pushing.type, Pushing.coinVol, f, isLine);
                if (org != now)
                    clear_refresh_counter();
            } else {
                ChartManager.getInstance().getChart().updateDataAndDisplay2();
...
Range.prototype.setRange = function(minValue, maxValue) {
    ...
    if (this._maxValue > this._minValue)
        ...
        this._ratio = 0;
        throw "data error: maxValue should be greater than minValue";
    }
...
MainDataSource.prototype.updateNew = function (data) {
    ...
    var cnt = data.length, len = this._dataItems.length;
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (e[0] == lastItem.date) {
            ...
            var isAllTheSame = lastItem.open == e[1] && lastItem.high == e[2] && lastItem.low == e[3] && lastItem.close == e[4] && lastItem.volume == e[5];
    ...
    if (!lastItemFound) {       // 未找到

        if (data[cnt - 1][0] < lastItem.date) {
            // 数据不合法 直接返回
            this.setUpdateMode(DataSource.UpdateMode.DoNothing);
            return false;
        }
    ...
    if (lastItemFound && !lastItemUpdated) {
        // 找到了最后一条数据但没更新
        this.setUpdateMode(DataSource.UpdateMode.DoNothing);
        return true;
    } else if (lastItemUpdated) {
        // 找到并更新了最后一条数据
        this.setUpdateMode(DataSource.UpdateMode.Update);
        return true;
    }
...
MainInfoPlotter.prototype.Draw = function(context) {
    ...
    var lastData = ds.getDataAt(selIndex - 1) || data;
    ...
    if (lang == "zh-cn") {
        if (!Plotter.drawString(context, '时间: ' +
            ...
            return;
        if (!Plotter.drawString(context, '  开: ' + data.open.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  高: ' + data.high.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  低: ' + data.low.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  收: ' + data.close.toFixed(GLOBAL_VAR.digits), rect))
            return;
    }
    else if (lang == "en-us") {
        if (!Plotter.drawString(context, 'DATE: ' +
                year + '-' + month + '-' + date + '  ' + hour + ':' + minute, rect))
            return;
        if (!Plotter.drawString(context, '  O: ' + data.open.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  H: ' + data.high.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  L: ' + data.low.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  C: ' + data.close.toFixed(GLOBAL_VAR.digits), rect))
            return;
    }
    else if (lang == "zh-tw") {
        if (!Plotter.drawString(context, '時間: ' +
                year + '-' + month + '-' + date + '  ' + hour + ':' + minute, rect))
            return;
        if (!Plotter.drawString(context, '  開: ' + data.open.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  高: ' + data.high.toFixed(GLOBAL_VAR.digits), rect))
            return;
        if (!Plotter.drawString(context, '  低: ' + data.low.toFixed(GLOBAL_VAR.digits), rect))
    ...
    var amplitude = (data.high - data.low) / lastData.close * 100.0;
    if (lang == "zh-cn") {
        if (!Plotter.drawString(context, '  振幅: ' + amplitude.toFixed(2) + ' %', rect))
            return;
        if (!Plotter.drawString(context, '  量: ' + data.volume.toFixed(GLOBAL_VAR.digits), rect))
            return;
    }
    else if (lang == "en-us") {
        if (!Plotter.drawString(context, '  AMPLITUDE: ' + amplitude.toFixed(2) + ' %', rect))
            return;
        if (!Plotter.drawString(context, '  V: ' + data.volume.toFixed(GLOBAL_VAR.digits), rect))
            return;
    }
    else if (lang == "zh-tw") {
        if (!Plotter.drawString(context, '  振幅: ' + amplitude.toFixed(2) + ' %', rect))
...
function KLineMouseEvent() {
    $(document).ready(function(){
        ...
        $("#chart_btn_parameter_settings").click(function() {
            ...
            //指标参数设置埋点记录
            window.parent&window.parent.eventLog({
                platform: 2,
                eventId: "click_indicatorset_charts"
            });
        });
        ...
        $('#chart_show_indicator')
            .click(function(){
                if ($(this).hasClass('selected')) {
                    ...
                    window.parent&&window.parent.eventLog({
                        platform: 2,
                        eventId: "click_telindicators_charts"
                    });
                }
            });
        $("#chart_tabbar li a")
        ...
        $("#chart_select_chart_style a")
            .click(function() {
                ...
                //主图样式埋点记录
                var chartStyleName = $(this).html();
                var eventId = "";
                switch (chartStyleName) {
                    case "CandleStick":
                        eventId = "click_mainpictype_candlestick_charts";
                        break;
                    case "CandleStickHLC":
                        eventId = "click_mainpictype_candlestickhlc_charts";
                        break;
                    case "OHLC":
                        eventId = "click_mainpictype_ohlc_charts";
                        break;
                }
                if(eventId !== "") {
                    window.parent&&window.parent.eventLog({
        ...
        $("#chart_select_main_indicator a")
            .click(function(){
                ...
                switch (name) {
                    case "MA":
                        eventId = "click_mainindicators_ma_charts";
                        break;
                    case "EMA":
                        eventId = "click_mainindicators_ema_charts";
                        break;
                    case "BOLL":
                        eventId = "click_mainindicators_boll_charts";
                        break;
                    case "SAR":
                        eventId = "click_mainindicators_sar_charts";
                        break;
                    case "NONE":
                        eventId = "click_mainindicators_off_charts";
                        break;
                }
                if(eventId !== "") {
                    window.parent&&window.parent.eventLog({
            ...
            success     : function(json) {
                if (GLOBAL_VAR.G_HTTP_REQUEST) {
                    ...
                    for(var i=0;i<jsonData.length;i++){
                        jsonArray.push([jsonData[i].createdDate,Number(jsonData[i].open),Number(jsonData[i].high),Number(jsonData[i].low),Number(jsonData[i].close),Number(jsonData[i].volume)]);
                    }
                    // var char1 = jsonArray.charAt(0);
                    // var char2 = jsonArray.charAt(1);
                    // if (char1 != "[" || char2 != "[") {
                    //     GLOBAL_VAR.TimeOutId = setTimeout(RequestData, 2000);
                    //     return;
                    // }
                    // var array = JSON.parse(jsonArray);
                    GLOBAL_VAR.KLineData = jsonArray;
                    if (ChartManager.getInstance().getChart()._money_type == 1) {
                        var rate = ChartManager.getInstance().getChart()._usd_cny_rate;
                        for (var i in GLOBAL_VAR.KLineData) {
                            var e = GLOBAL_VAR.KLineData[i];
...
function switch_theme(name) {
    ...
    if (name == 'dark') {
        ...
        eventId = "click_theme_black_charts";
    } else if (name == 'light') {
        $('#chart_toolbar_theme').find('a').each(function() {
        ...
        eventId = "click_theme_white_charts";
    }
    var a = {};

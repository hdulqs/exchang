...
Kline.prototype = {
    ...
    websocketRedister: function(symbol) {
		...
		var target = "ws://localhost:8010/front/websocket";
		
		if ("WebSocket" in window) {
			this.klineWebsocket = new WebSocket(target)
		} else {
			if ("MozWebSocket" in window) {
				this.klineWebsocket = new MozWebSocket(target)
			} else {
				return
			}
		}
		...
		//接收到消息的回调方法
		this.klineWebsocket.onmessage = function(result) {
			...
			if (data.trades) {
				$this.pushTrades(data.trades);
				$this.klineTradeInit = true
			}
			if (data.depths) {
				$this.updateDepth(data.depths)
			}
		...
		//些方法供client.js调用
		var klineMessage = function(result) {
        ...
        function  loadTradeJson() {
            $.ajax({
                type: 'get',
                //url:"http://localhost:9002/static/js/trade/trade.json",
                url :"http://www.batsoft.com/kline/tradeJson",
                dataType: "json",
                klineMessage : klineMessage,
                success: function(data){

                    klineMessage(data.pushEntrusMarket);
                    klineMessage(data.pushNewListRecordMarket);
                }
            });
        }

        setInterval(loadTradeJson, 1000);
		
		this.klineWebsocket.onopen = function() {
			console.log("kline_websocket连接成功");
		};
		//连接发生错误
		this.klineWebsocket.onerror = function() {
		};
		//连接关闭
		this.klineWebsocket.onclose = function() {
		}

...
public class SocketBusServiceImpl implements SocketBusService {
    ...
    @Override
	public void pushKlineData(String symbol, String subject, String channel) {
    ...
    public void wsInitTicker() {
        ...
        for (String coinPair : list) {
            ...
            if (StringUtils.hasText(value)) { 	//["BTC_USDT 【交易对】","","","","","","378290.00【24h涨跌】 ","3783.89990【 最新价】","47400.74875 【24h成交量】","3961.16010 【24h 最高价】","37.0 【24h 最低价】","","","3942.49990","26033.23"]
            	String pricingCoinCode = coinPair.split(CHS.underline.getValue())[1];
            	if(!Objects.equals(CoinEnum.USDT.getCode(), pricingCoinCode)) {
                    BigDecimal bigDecimal = RedisUserUtil.getKlineCovertToUSDT(pricingCoinCode, CoinEnum.USDT.getCode()).setScale(2, BigDecimal.ROUND_HALF_EVEN);
                    JSONArray strArray = JSONArray.parseArray(value);
                    strArray.set(14, bigDecimal);
                    simpMessagingTemplate.convertAndSend(ws_to, new Response(strArray.toString()));
                }else {
                    simpMessagingTemplate.convertAndSend(ws_to, new Response(value));
                }
            }
        }
    }

var GLOBAL_VAR = {
  ...
  //  url:"http://127.0.0.1:8889/static/front/src/script/trade/data.json"
    url : "http://www.batsoft.com/kline/data"
    //url: HRY.host+HRY.modules.spotmain+"kline/data"
...
TableLayout.prototype.measure = function(a, u, m) {
    ...
    if (e.getFirstIndex() >= 0) {
        ...
        for (C = 0,
        j = 0,
        f = z; C < this._areas.length && j < l.length; C += 2) {
            var A = this._areas[C];
    ...
    $(document).ready(GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
        ...
        success: function(json) {
            if (GLOBAL_VAR.G_HTTP_REQUEST) {
                ...
                if (!json.success) {
                    alert(json.msg);
                    kline.refreshPage(json.obj.ecode == 101 ? null  : GLOBAL_VAR.market_from);
                    return
                }
                GLOBAL_VAR.market_from_name = json.obj.marketName;
                var chart = ChartManager.getInstance().getChart();
                chart._contract_unit = json.obj.contractUnit;
                chart._money_type = json.obj.moneyType;
                chart._usd_cny_rate = json.obj.USDCNY;
                chart.setTitle();
                kline.setMarketShow(GLOBAL_VAR.market_from_name, chart._contract_unit, chart._money_type, json.obj.url);
                kline.setTopTickers(json.obj.topTickers);
                GLOBAL_VAR.KLineData = eval(json.obj.data);
                try {
...
function on_size() {
	...
	var u = window.innerWidth - $("#trade_container:visible").innerWidth();

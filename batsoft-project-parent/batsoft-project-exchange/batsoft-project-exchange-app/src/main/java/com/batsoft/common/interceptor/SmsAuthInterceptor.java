 ...
 * @author simon
 */
public class SmsAuthInterceptor extends HandlerInterceptorAdapter {
	
	private static final String REDIS_FOLDER = "AUTH:SMS:";
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		if(handler.getClass().equals(HandlerMethod.class)) {
			SmsAuth passport = ((HandlerMethod)handler).getMethodAnnotation(SmsAuth.class);
			if(passport != null && Objects.equals(passport.value(), SmsAuth.SIMPLE_PATTERN)) {
				return simplePattern(request, response, handler);
			}
			if(passport != null && Objects.equals(passport.value(), SmsAuth.ADD_NODE)) {
				this.addNode(request, response, handler);
			}
			if(passport != null && Objects.equals(passport.value(), SmsAuth.CHECK_SEND_SMS)){
				return checkSendSms(request, response, handler);
			}
		}
		return Boolean.TRUE;
	}
	 ...
	 * @throws Exception 
	 */
	private boolean checkSendSms(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		// IP地址
		String ip = ipMotion.getIpAddr(request);
		...
		if(param == null) {
			...
			if(endTime != null && diffMotion.dateDiffSecond(beginTime, endTime) > 0) {
				...
				if(count >= MAX_COUNT) {
					param.setBeginTime(beginTime);
					param.setEndTime(mathMotion.dateAddSecond(beginTime, DISABLED_TIME));
					param.setCount(BigInteger.ZERO.intValue());
				}else {
					param.setCount(++count);
					param.setBeginTime(null);
					param.setEndTime(null);

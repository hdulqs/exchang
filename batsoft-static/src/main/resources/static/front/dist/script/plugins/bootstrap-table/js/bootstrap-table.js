...
define(function(require, exports, module) {
    ...
    (function($) {
        ...
        var getScrollBarWidth = function() {
            if (cachedWidth === null) {
                var inner = $('<p/>').addClass('fixed-table-scroll-inner'), outer = $('<div/>').addClass('fixed-table-scroll-outer'), w1, w2;
        ...
        var compareObjects = function(objectA, objectB, compareLength) {
            ...
            if (compareLength) {
                // If number of properties is different, objects are not
                // equivalent
            ...
            for (var i = 0; i < objectAProperties.length; i++) {
                ...
                // If the property is not in the object B properties, continue
        ...
        BootstrapTable.prototype.initLocale = function() {
            if (this.options.locale) {
                ...
                if ($.fn.bootstrapTable.locales[this.options.locale]) {
                    ...
                    // locale with sep set to - (in case original was specified
                    // with _)
        ...
        BootstrapTable.prototype.initTable = function() {
            ...
            this.$el.find('>tbody>tr').each(function(y) {
                ...
                $(this).find('>td').each(function(x) {
                    ...
                    for (tx = x; tx < x + cspan; tx++) { // mark matrix
                        // elements occupied
                        // by current cell
                        // with true
            ...
            $.each(this.options.columns, function(i, columns) {
                ...
                $.each(columns, function(j, column) {
                    var text = '', halign = '', // header align style
                        ...
                        style = '', class_ = sprintf(' class="%s"', column['class']), order = that.options.sortOrder || column.order, unitWidth = 'px', width = column.width;
        ...
        BootstrapTable.prototype.initSort = function() {
            var that = this, name = this.options.sortName, order = this.options.sortOrder === 'desc' ? -1 : 1, index = $.inArray(this.options.sortName, this.header.fields);
            ...
            if (index !== -1) {
                ...
                this.data.sort(function(a, b) {
                    ...
                    var aa = getItemField(a, name, that.options.escape), bb = getItemField(b, name, that.options.escape), value = calculateObjectValue(that.header, that.header.sorters[index], [ aa, bb ]);
        ...
        BootstrapTable.prototype.initToolbar = function() {
            ...
            if (this.options.search) {
                 ...
                 */
                var html_a = [];
                html_a.push(sprintf('<div  class="search pull-right" style="margin-right:5px;"> ') + sprintf('<select id="sel_column" class="form-control"> '));
                ...
                html_a.push(sprintf('</select>') + sprintf('</div>')
...
//					that.trigger('search', text_t);
        ...
        BootstrapTable.prototype.initPagination = function() {
            ...
            var that = this, html = [], $allSelected = false, i, from, to, $pageList, $first, $pre, $next, $last, $number, data = this.getData(), pageList = this.options.pageList;
            ...
            if (this.options.totalRows) {
                if (this.options.pageSize === this.options.formatAllRows()) {
                    ...
                    // multiple pages and a search that matches to one page
                    // throws exception
        ...
        BootstrapTable.prototype.initBody = function(fixedScroll) {
            ...
            for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
                ...
                $.each(this.header.fields, function(j, field) {
                    ...
                    if (column.checkbox || column.radio) {
                        ...
                        // Hide empty data on Card view when smartDisplay is set
                        // to true.
            ...
            /*if($("input[type='checkbox']").iCheck){
                $("input[type='checkbox']").iCheck({
                    checkboxClass: 'icheckbox_square-green',
                    radioClass: 'iradio_square-green',
                });

                var checkAll = $("#table input[name='btSelectAll']:checkbox");
                var checkboxes = $("#table input[name='btSelectItem']:checkbox");

                checkAll.on('ifChecked ifUnchecked', function(event) {
                    if (event.type == 'ifChecked') {
                        checkboxes.iCheck('check');
                        checkAll.attr('checked',true);//checked为true时为默认显示的状态
                        // that.checkAll(true);
                    } else {
            ...
            this.$body.find('> tr[data-index] > td').off('click dblclick').on('click dblclick', function(e) {
                var $td = $(this), $tr = $td.parent(), item = that.data[$tr.data('index')], index = $td[0].cellIndex, fields = that.getVisibleFields(), field = fields[that.options.detailView && !that.options.cardView ? index - 1 : index], column = that.columns[getFieldIndex(that.columns, field)], value = getItemField(item, field, that.options.escape);
        ...
        BootstrapTable.prototype.initServer = function(silent, query, url) {
            ...
            if (this.options.queryParamsType === 'limit') {
                ...
                if (this.options.pagination) {
                    params.page = this.options.pageSize === this.options.formatAllRows() ? 0 : this.options.pageSize * (this.options.pageNumber - 1);
                    params.pageSize = this.options.pageSize === this.options.formatAllRows() ? this.options.totalRows : this.options.pageSize;
                }
            }
            ...
            if(this.options.search){
                //添加上自定义的下拉框名称
                params.mycolumn = $('#sel_column').val();
            }
        ...
        BootstrapTable.prototype.resetFooter = function() {
            ...
            $.each(this.columns, function(i, column) {
                var key, falign = '', // footer align style
                    valign = '', csses = [], style = {}, class_ = sprintf(' class="%s"', column['class']);
        ...
        BootstrapTable.prototype.resetView = function(params) {
            ...
            if (this.options.height) {
                var toolbarHeight = getRealHeight(this.$toolbar), paginationHeight = getRealHeight(this.$pagination), height = this.options.height - toolbarHeight - paginationHeight;
        ...
        BootstrapTable.prototype.getRowByUniqueId = function(id) {
            ...
            for (i = len - 1; i >= 0; i--) {
                ...
                } else if (row._data.hasOwnProperty(uniqueId)) { // uniqueId
                    // is a row
                    // data
                    // property
        ...
        BootstrapTable.prototype.mergeCells = function(options) {
            var row = options.index, col = $.inArray(options.field, this.getVisibleFields()), rowspan = options.rowspan || 1, colspan = options.colspan || 1, i, j, $tr = this.$body.find('>tr'), $td;
        ...
        BootstrapTable.prototype.destroy = function() {
            ...
            this.$el.html(this.$el_.html()).css('margin-top', '0').attr('class', this.$el_.attr('class') || ''); // reset
            // the
            // class
        };
        ...
        BootstrapTable.prototype.refreshOptions = function(options) {
            // If the objects are equivalent then avoid the call of destroy /
            // init methods

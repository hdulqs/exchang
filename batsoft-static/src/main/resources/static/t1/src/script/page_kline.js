   ...
   var U = require('./module_utils');
    function init() {
        ...
        Chart.prototype.__construct = function () {
            ...
            this._money_type = "ETH";
            this._contract_unit = "EOS";
            this.strIsLine = false;
            this.strCurrentMarket = 20180214000;
        ...
        Chart.prototype.updateDataAndDisplay = function () {
            ...
            this.setTitle();
            ChartManager.getInstance().setCurrentDataSource("frame0.k0", "GGCOIN." + this._market_from + "." + this._time);
        ...
        ChartManager.prototype.init = function () {
            ...
            DefaultTemplate.loadTemplate("frame0.k0", "GGCOIN");
        ...
        MainDataSource.prototype.calcDecimalDigits = function (a) {
            var c = Number(a).toFixed(8).replace(/\.?0+$/, "");
            var b = c.indexOf(".");
            if (b < 0) {
                return 0
            }
        ...
        DarkTheme.prototype.__construct = function () {
            this._colors = [];
            this._colors[Theme.Color.Positive] = "#195A28";
            this._colors[Theme.Color.Negative] = "#642024";
            this._colors[Theme.Color.PositiveDark] = "#004718";
            this._colors[Theme.Color.NegativeDark] = "#272727";
            ...
            this._colors[Theme.Color.Indicator1] = "#0066d4";
            ...
            this._colors[Theme.Color.Grid0] = "#444";
            ...
            this._colors[Theme.Color.TextPositive] = "#96b105";
            this._colors[Theme.Color.TextNegative] = "#ff175e";
        ...
        CandlestickPlotter.prototype.Draw = function (c) {
            ...
            for (var t = g; t < n; t++) {
                ...
                if (r > o) {
                    ...
                    if (B.high > r) {
                        m = Math.min(m, q - 1);
                        f.push({x: z, y: m, w: 1, h: q - m})
                    }
                    if (o > B.low) {
                        C = Math.max(C, k + 1);
                        f.push({x: z, y: k, w: 1, h: C - k})
                    }
                } else {
                    if (r == o) {
                        var q = p.toY(r);
        ...
        OHLCPlotter.prototype.Draw = function (b) {
            ...
            for (var r = f; r < n; r++) {
                ...
                if (A.close > A.open) {
                    var p = o.toY(A.close);
                    var j = o.toY(A.open);
                    e.push({x: x, y: m, w: 1, h: c});
                    e.push({x: d, y: p, w: v, h: 1});
                    e.push({x: x, y: j, w: v, h: 1})
                } else {
                    if (A.close == A.open) {
                        var l = o.toY(A.close);
                        a.push({x: x, y: m, w: 1, h: c});
                        a.push({x: d, y: l, w: v, h: 1});
                        a.push({x: x, y: l, w: v, h: 1})
                    } else {
                        var p = o.toY(A.open);
                        var j = o.toY(A.close);
                        k.push({x: x, y: m, w: 1, h: c});
                        k.push({x: d, y: p, w: v, h: 1});
                        k.push({x: x, y: j, w: v, h: 1})
                    }
                }
                d += t;
                x += t;
        ...
        MainInfoPlotter.prototype.Draw = function (c) {
            ...
            var a = {x: w.getLeft() + 4, y: w.getTop() + 2, w: w.getWidth() - 8, h: 20};
            ...
            for (var l = e; l < h; l++) {
                ...
                if (m >= 0) {
                    ...
                    if ((l == 0 || m >= j) && b > 1 && q > 1) {
                        ...
                        d.push({x: c, y: s - b, w: Math.max(q, 1), h: Math.max(b, 1)})
                    }
                } else {
                    var b = k.toHeight(-m);
                    if ((l == 0 || m >= j) && b > 1 && q > 1) {
                        ...
                        f.push({x: c, y: s, w: Math.max(q, 1), h: Math.max(b, 1)})
                    }
                }
                j = m;
                c += o
        ...
        IndicatorInfoPlotter.prototype.Draw = function (c) {
            ...
            for (e = 0; e < d; e++) {
                ...
                f = "  " + i.getName() + ": " + String.fromFloat(o, 6);
                g = i.getColor();
                if (g === undefined) {
        ...
        RangePlotter.prototype.Draw = function (c) {
            ...
            for (var d in l) {
                ...
                c.fillText(String.fromFloat(l[d], 8), a, k)
            }
            if (f.length > 0) {
        ...
        TimelineSelectionPlotter.prototype.Draw = function (b) {
            ...
            b.fillRect(g - 52.5, l.getTop() -30, 106, 22);
            b.strokeStyle = m.getColor(Theme.Color.Grid3);
            b.strokeRect(g - 52.5, l.getTop() -30, 106, 22);
        ...
        RangeSelectionPlotter.prototype.Draw = function (b) {
            ...
            Plotter.createPolygon(b, [{x: a.getLeft(), y: g}, {x: a.getLeft() + 5, y: g + 10}, {
                x: a.getRight() - 3,
                y: g + 10
            }, {x: a.getRight() - 3, y: g - 10}, {x: a.getLeft() + 5, y: g - 10}]);
        ...
        ChartSettings.init = function () {
            ...
            d.indicsStatus = "open";
            d.period = l;
            ChartSettings._data = {ver: 1, charts: d, indics: f, theme: "Dark"};
            ChartSettings.checkVersion()
        };
        ...
        function KLineMouseEvent() {
            $(document).ready(function () {
                ...
                a();
                $(window).resize(a);
                $("#chart_overlayCanvas").bind("contextmenu", function (b) {
                    b.cancelBubble = true;
                    b.returnValue = false;
                    b.preventDefault();
                    b.stopPropagation();
        ...
        var RequestData = function (showLoading) {
            ...
            $(document).ready(function () {
                var stompClient;
                var url=GLOBAL_VAR.url,
                    time=GLOBAL_VAR.time_type,
                    market= GLOBAL_VAR.market_from;

                function send(){
                    stompClient.send("/init_kline", {}, market + "@" + time);
                }

                function message(e){
                    if (Loading['kline'] == undefined) {
                        Loading['kline'] = 1;
                    }
                        ...
                        json = json.map(function (v) {
                            return [v[0], parseFloat(v[1]), parseFloat(v[3]), parseFloat(v[4]), parseFloat(v[2]), parseFloat(v[5])];
                        })
                        GLOBAL_VAR.KLineData = json;
                ...
                stompClient.connect({}, function(frame) {
                    ...
                    stompClient.subscribe("/exchange/market/" +market +'@kline_'+time, function(respnose){ //订阅/topic/getResponse 目标发送的消息。这个是在控制器的@SendTo中定义的。
                        message(JSON.parse(respnose.body));
                    });
               ...
               /* var ws = new WebSocket(url + market + '@kline_' + time);
                ws.time = time;
                ws.market = market;
                ws.onopen = function () {
                    console.log("kline onopen");
                };
                ws.onclose = function () {
                    if (ws.time != GLOBAL_VAR.time_type || ws.market != GLOBAL_VAR.market_from) {
                        return;
                    }
                    GLOBAL_VAR.TimeOutId = setTimeout(function () {
                        console.log("kline RequestData");
                        RequestData(true)
                    }, 5000)
                };
        ...
        function AbortRequest() {
            if (GLOBAL_VAR.G_HTTP_REQUEST && GLOBAL_VAR.G_HTTP_REQUEST.readyState != WebSocket.CLOSED) {
                GLOBAL_VAR.G_HTTP_REQUEST.close()
            }
        }
        ...
        function refreshTemplate() {
            GLOBAL_VAR.chartMgr = DefaultTemplate.loadTemplate("frame0.k0", "GGCOIN");
            ChartManager.getInstance().redraw("All", true)
        }
        ...
        function on_size() {
            var u = $('.chartBox').width();
            var t = Math.max(100, window.innerHeight - $("#mainBottom").height() - $(".header").height() - 10);
            var o = $("#chart_container");
            o.css({width: u + "px", height: t + "px"});
            var z = $("#chart_toolbar");
            ...
            if (G) {
                p.css({left: f.x + "px", top: f.y + "px", width: f.w + "px", height: f.h + "px"})
            }
            ...
            g.height = w.h;
            if (n) {
                B.css({left: A.x + "px", top: A.y + "px", width: A.w + "px", height: A.h + "px"})
            }
            var j = $("#chart_parameter_settings");
            j.css({left: (u - j.width()) >> 1, top: (t - j.height()) >> 1});
            var c = $("#chart_loading");
        ...
        function mouseWheel(a, b) {
            ChartManager.getInstance().scale(a.originalEvent.wheelDelta > 0 || a.originalEvent.detail < 0 ? 1 : -1);
            ChartManager.getInstance().redraw("All", true);
            return false
        }
